"""
Long Call Strategy Generator.

Generates long call candidates for bullish outlook.
"""
...
def generate_long_calls(
    contracts: List[OptionContract],
    budget: float,
    spot_price: float,
) -> List[StrategyCandidate]:
    """
    Generate long call candidates.
    ...
    Returns:
        List of ranked long call candidates
    """
    # Filter calls that meet criteria
    calls = [
    ...
    for idx, c in enumerate(calls):
        candidate = _build_long_call_candidate(c, idx, spot_price)
...
def _build_long_call_candidate(
    contract: OptionContract,
    idx: int,
    spot_price: float,
) -> StrategyCandidate:
    """Build a single long call candidate from a contract."""
    ...
    scores = _calculate_long_call_scores(contract, breakeven_price, spot_price)
    ...
    candidate: StrategyCandidate = {
        "id": f"long-call-{idx}-{contract['contractSymbol']}",
...
def _calculate_long_call_scores(
    contract: OptionContract,
    breakeven: float,
    spot_price: float,
) -> Dict[str, int]:
    """
    Calculate scores for long call candidate.

    Metrics:
    - breakevenHurdle: How far stock needs to move to break even (lower = better)
    ...
    """
    return {
        "breakevenHurdle": calculate_breakeven_score(breakeven, spot_price, is_call=True),
...
def _calculate_overall_score(scores: Dict[str, int]) -> float:
    ...
    Weights:
    - Breakeven Hurdle: 30% (most important - how much does stock need to move?)
...
def _generate_reasons(
    ...
    if delta >= 0.6:
        reasons.append(f"High delta ({delta:.2f}) - strong directional exposure")
    elif delta >= 0.5:
        reasons.append(f"ATM delta ({delta:.2f}) - balanced risk/reward")
    else:
        reasons.append(f"OTM delta ({delta:.2f}) - leveraged exposure")

    # Time value
    reasons.append(f"{contract['dte']} DTE provides time for move")
